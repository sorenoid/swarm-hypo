...
/**
 * This dialog that embeds on the top search dialog and on the bottom table that
 * includes results of search in database. All components in search dialog can
 * be traversed by Tab. This dialog allows :
 * <ul>
 * <li>
 * As soon as focus is lost on text field its text is validated and if it is
 * invalid text field background color is changed to red</li>
 * <li>
 * At any moment user can press ESCAPE button to hide the dialog. If user again
 * wants to show dialog, Ctrl+S should be pressed. Changes to search text fields
 * are not lost.</li>
 * <li>
 * Press Enter or Alt+S to perform search after search data was entered.</li>
 * <li>
 * Press Ctrl+D to maximize search dialog. If divider is on top and previously
 * it was not on bottom then it is needed to press Ctrl+D twice.</li>
 * <li>
 * Press Ctrl+U to maximize search result dialog. If divider is on bottom and
 * previously it was not on top then it is needed to press Ctrl+U twice.</li>
 * <li>
 * Press Alt+Enter to resize whole dialog to full screen. To escape from full
 * screen press Alt+Enter again</li>
 * <li>
 * Press Ctrl+J to enter attempt_id column's value which will be selected in
 * search result table. It means that you press Ctrl+J and then you type
 * attempt_id value and you get corresponding preferred column selected. No need
...
public class DataSearchDialog extends JDialog
{
	...
	private JPanel createSearchPanel()
	{
		...
		eventTypeField = this.createDataSearchTextField(new DataSearchTextField<String>(String.class, "event label",
				30, Color.GRAY));
		timestampLabel = new JLabel("Timestamp : ");
		timestammpToLabel = new JLabel("to");
		timestampStartField = this.createDataSearchTextField(new DataSearchTextField<Timestamp>(Timestamp.class,
	...
	private void doSearch(SearchQueryCriteria searchQueryCriteria) throws SQLException
	{
		if(searchByEventCheckBox.isSelected()){
			...
			dataSearchResultPanel.isEventList = false;
			List<Attempt> attempts = Marker.getAttemptsByPMarker(eventTypeField.validValue(), timestampStartField.validValue(), timestampEndField.validValue());
			dataSearchResultPanel.updateDataSearchTableModel(attempts);
		}
