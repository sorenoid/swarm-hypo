    ...
    public void render(Graphics2D g)
    {
        ...
        for (int j = 0; j < yBins; j++)
        {
            ...
            for (int i = 0; i < xBins; i++)
            {
                ...
                //double binWidth = histogram.xAxis().binWidth(i);
                rect.setRect(getXPixel(histogram.xAxis().lowerEdge() + xOffset), getYPixel(histogram.yAxis().lowerEdge() + yOffset),
                        binWidth * getXScale(), binHeight * getYScale());
                ...
                g.setPaint(spectrum.colors[color]);
                g.fill(rect);
                //offset += binWidth;
