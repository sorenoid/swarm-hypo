...
public class WMSGeoImageSet extends GeoImageSet {
	...
	public enum ExceptionType {
		XML("application/vnd.ogc.se_xml"), IN_IMAGE(
				"application/vnd.ogc.se_inimage"), BLANK(
	...
	private void getReader(String resource) throws MalformedURLException,
			IOException {
	...
	@Override
	public synchronized GeoImage getCompositeImage(final GeoRange range,
			final int ppdLon, final int ppdLat, final double scale) {
		GeoImage ret = null;
		try {
			Retriable<GeoImage> rt = new Retriable<GeoImage>("WMS", 3) {
				...
				public boolean attempt() throws UtilException {
					...
					char c = server.indexOf('?') == -1 ? '?' : '&';
					String request = String.format("%s%cSERVICE=WMS"
							+ "&VERSION=1.1.1" + "&REQUEST=GetMap"
							+ "&LAYERS=%s" + "&STYLES=%s" + "&WIDTH=%d"
							+ "&HEIGHT=%d" + "&FORMAT=%s" + "&EXCEPTIONS=%s"
							+ "&SRS=%s" + "&BBOX=%.5f,%.5f,%.5f,%.5f",
							server,
							c,
							layer, // "BMNG"
							styles, (int) width, (int) height, imageType.mime,
							exceptionType.mime, EPSG_4326, w, range.getSouth(),
							e, range.getNorth());
					...
					try {
						...
						BufferedImage copy = new BufferedImage(im.getWidth(),
						...
						try {
							inputStream.reset();
							String s;
							logger.warning("WMS failure: " + ex.getMessage());
							while ((s = in.readLine()) != null)
								logger.warning("\t" + s);
