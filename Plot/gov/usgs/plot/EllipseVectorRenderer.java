...
{
	protected final static Logger logger = Log.getLogger("gov.usgs.plot.EllipseVectorRenderer"); 
	...
	*/
	...
	private static final double[] SCALES = new double[] 
		{100000, 50000, 20000, 10000, 5000, 2000, 1000, 500, 200, 100, 50, 20, 10, 5, 2, 1, 0.5, 0.2, 0.1, 0.05, 0.02, 0.01};  
	...
	public void render(Graphics2D g)
	{
		...
		if (displayVert){
			line.setLine(frameRenderer.getXPixel(x), frameRenderer.getYPixel(y), frameRenderer.getXPixel(x), frameRenderer.getYPixel(y + z * scale));
			...
			if (drawEllipse){
				...
				g.setColor(colorVert);
				//logger.fine(z + " " + sigZ);
				line.setLine(frameRenderer.getXPixel(x) - 3, frameRenderer.getYPixel(y + (z - sigZ) * scale), frameRenderer.getXPixel(x) + 3, frameRenderer.getYPixel(y + (z - sigZ) * scale));
				g.draw(line);
				line.setLine(frameRenderer.getXPixel(x) - 3, frameRenderer.getYPixel(y + (z + sigZ) * scale), frameRenderer.getXPixel(x) + 3, frameRenderer.getYPixel(y + (z + sigZ) * scale));
				g.draw(line);
			}
		...
		if (displayHoriz)
		{
			...
			line.setLine(frameRenderer.getXPixel(x), frameRenderer.getYPixel(y), frameRenderer.getXPixel(x + u * scale), frameRenderer.getYPixel(y + v * scale));
			//logger.fine("render - x:" + frameRenderer.getXPixel(x) + "/y:" + frameRenderer.getXPixel(y) + "/u:" + frameRenderer.getXPixel(x + u * scale) + "/v:" + frameRenderer.getXPixel(y + v * scale));
			...
			if (drawEllipse){
				...
				g.setStroke(origStroke);
				g.setColor(colorHoriz);
				Ellipse2D.Double e = new Ellipse2D.Double(0, 0, ellipseWidth * scale, ellipseHeight * scale);
				e.x = -e.width / 2;
				e.y = -e.height / 2;
