...
/**
 * <p>Plot is the top level class that contains a Valve plot.  The Valve plotting
 ...
 * responsibility of each of these Renderers to leave the Graphics2D object 
 * that they render upon in the same state in which they got it.  This way
 * a Renderer only has to worry about itself.</p>
 *
 * <p>A FrameRenderer (the class that all Valve graphs wrap their data in) can
 * in theory be put anywhere on the plot.  But because Valve allows users to 
 * overlay plots there is a mechanism called the Default Frame Renderer Location
 * that specifies where in the plot the Frame Renderer should be placed.  The
 * Valve grapher sets this before each of the GraphLines are rendered depending
 * on the size of the graph and other factors.  Each frame renderer then places
 * itself where the plot tells it to via setFrameRendererLocation(FrameRenderer).
 * Also set by the grapher is the default number of ticks on each axis.</p>
 *
 * $Log: not supported by cvs2svn $
 * Revision 1.3  2005/10/14 18:02:54  dcervelli
 * Added writeJPEG.
 *
 * Revision 1.2  2005/08/29 15:53:13  dcervelli
 * Added new constructor and getBackgroundColor.
 *
...
{
	protected final static Logger logger = Log.getLogger("gov.usgs.plot.Plot"); 
...
//            this.render(imageGraphics);
    ...
    public void writePS(String fn)
    {
        ...
        try
        {
            ...
            p.setSize(612, 792);
            p.setImageableArea(0,0, 612, 792);
            pf.setPaper(p);
            book.append(this, pf);
    ...
    public void render(Graphics2D g) throws PlotException
    {
        ...
        //g.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDERING_QUALITY);
	...
	/** Shows the plot on the screen at 0,0 location. This is typically used for debugging
	 * purposes.
	 */
	 ...
	 * purposes.
	 * @param x x-coord of location of plot
	 * @param y y-coord of location of plot
	 */
	...
	/** Utility function for iterating through an array of Renderers
	 * and rendering them while checking for nulls.
	 * @param g the Graphic2D to render upon
	 * @param renderers the array of Renderers
	 */
