...
public class MatrixRenderer extends BasicFrameRenderer {
	...
	public void render(Graphics2D g) {
        ...
        for (int j = 0; j < data.columns() - offset; j++) {
            ...
            if (sr != null) {
                ...
                gp.reset();
                gp.moveTo((float)getXPixel(data.getQuick(0, 0)), (float)(getYPixel(data.getQuick(0, j + offset))));
            }
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < size; i++)
		{
			...
			d.setQuick(i, 3, Math.cos(8 * Math.PI * ((double)i / size)) + 0.25 * Math.sin(28 * Math.PI * ((double)i / size)));
		}
