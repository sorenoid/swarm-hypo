...
//        double maxY = frameRenderer.getMaxY();
        ...
        if (majorTicks != null)
        {
	        for (int j = 0; j < majorTicks.length; j++)
	        {
	            ShapeRenderer sr = new ShapeRenderer(
	                    new Line2D.Double(frameRenderer.getXPixel(majorTicks[j]), frameRenderer.getYPixel(minY), 
	                    frameRenderer.getXPixel(majorTicks[j]), frameRenderer.getYPixel(minY) - majorLength));
	            if (color != null)
	            	sr.color = color;
	            bottomTicks[i++] = sr;
        ...
        if (minorTicks != null)
        {
	        ...
	        {
	            ShapeRenderer sr = new ShapeRenderer(
	                    new Line2D.Double(frameRenderer.getXPixel(minorTicks[j]), frameRenderer.getYPixel(minY), 
	                    frameRenderer.getXPixel(minorTicks[j]), frameRenderer.getYPixel(minY) - minorLength));
...
//            tr.transformer = frameRenderer;
    ...
    public void createTopTicks(double[] majorTicks, double[] minorTicks, Color color)
    {
        ...
        if (majorTicks != null)
        {
		    ...
		    {
		        ShapeRenderer sr = new ShapeRenderer(
		                new Line2D.Double(frameRenderer.getXPixel(majorTicks[j]), frameRenderer.getYPixel(maxY), 
		                frameRenderer.getXPixel(majorTicks[j]), frameRenderer.getYPixel(maxY) + majorLength));
		        if (color != null)
		        	sr.color = color;
		        topTicks[i++] = sr; 
		    }
        ...
        if (minorTicks != null)
        {
	        ...
	        {
	            ShapeRenderer sr = new ShapeRenderer(
	                    new Line2D.Double(frameRenderer.getXPixel(minorTicks[j]), frameRenderer.getYPixel(maxY), 
	                    frameRenderer.getXPixel(minorTicks[j]), frameRenderer.getYPixel(maxY) + minorLength));
	            if (color != null)
		        	sr.color = color;
		        topTicks[i++] = sr;
	        }
...
//        double maxX = frameRenderer.getMaxX();
        for (int i = 0; i < ticks.length; i++)
        {
            ...
            ShapeRenderer sr = new ShapeRenderer(new Line2D.Double(frameRenderer.getXPixel(minX), frameRenderer.getYPixel(value),
...
//	            tr.transformer = frameRenderer;
    ...
    public void createLeftTickLabels(double[] ticks, String[] labels, Color color)
    {
        if (labels == null)
        {
            ...
            double exp = gov.usgs.math.Util.getExp(max);
            boolean reformat = (exp >= 5 || exp <= -4);
            for (int i = 0; i < ticks.length; i++){
                labels[i] = (reformat ? numberFormat.format(ticks[i] / Math.pow(10, exp)) : numberFormat.format(ticks[i]));
            } 
            if (reformat)
            {
                TextRenderer tr = new TextRenderer();
...
//        double minX = frameRenderer.getMinX();
        ...
        {
            double value = ticks[i];
            ShapeRenderer sr = new ShapeRenderer(new Line2D.Double(frameRenderer.getXPixel(maxX) - width, frameRenderer.getYPixel(value),
...
//        tr.transformer = frameRenderer;
    ...
    public void render(Graphics2D g)
    {
        ...
        if (backgroundColor != null)
        {
            ...
            g.fill(new Rectangle(frameRenderer.getGraphX(), frameRenderer.getGraphY(), frameRenderer.getGraphWidth(), frameRenderer.getGraphHeight()));
