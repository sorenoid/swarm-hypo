...
public class TransverseMercator extends Projection {
	...
	public boolean equals(Object obj) {
		...
		return tm.falseEasting == falseEasting
				&& tm.falseNorthing == falseNorthing
	...
	public static TransverseMercator fromUTM(String utm) {
		...
		try {
			...
			int zoneNumber = Integer
					.parseInt(utm.substring(0, utm.length() - 1));
	...
	private GeoRange getSouthGeoRange(double left, double right, double bottom,
			double top) {
		double dx = right - left;
		double dy = top - bottom;

		Point2D.Double c = inverse(new Point2D.Double(left + dx / 2, bottom
				+ dy / 2));
	...
	private GeoRange getNorthGeoRange(double left, double right, double bottom,
			double top) {
		double dx = right - left;
		double dy = top - bottom;
	...
	public GeoRange getGeoRange(double left, double right, double bottom,
			double top) {
		if (origin.y >= 0)
	...
	public Point2D.Double[] forward(Point2D.Double[] lonLat) {
		...
		epsq = esq / (1 - esq);
		MO = a
				* ((1 - esq / 4 - 3 * esq * esq / 64 - 5 * esq * esq * esq
						/ 256)
						* phiO
						- (3 * esq / 8 + 3 * esq * esq / 32 + 45 * esq * esq
								* esq / 1024)
						* Math.sin(2 * phiO)
						+ (15 * esq * esq / 256 + 45 * esq * esq * esq / 1024)
						* Math.sin(4 * phiO) - (35 * esq * esq * esq / 3072)
						* Math.sin(6 * phiO));
		...
		for (int i = 0; i < lonLat.length; i++) {
			...
			M = a
					* ((1 - esq / 4 - 3 * esq * esq / 64 - 5 * esq * esq * esq
							/ 256)
							* phi
							- (3 * esq / 8 + 3 * esq * esq / 32 + 45 * esq
									* esq * esq / 1024)
							* Math.sin(2 * phi)
							+ (15 * esq * esq / 256 + 45 * esq * esq * esq
									/ 1024) * Math.sin(4 * phi) - (35 * esq
							* esq * esq / 3072)
							* Math.sin(6 * phi));
			x = scale
					* N
	...
	public Point2D.Double[] inverse(Point2D.Double[] points) {
		...
		for (int i = 0; i < points.length; i++) {
			...
			M = MO + y / scale;
			mu = M
					/ (a * (1 - esq / 4 - 3 * esq * esq / 64 - 5 * esq * esq
							* esq / 256));

			phi1 = mu + (3 * e1 / 2 - 27 * e1 * e1 * e1 / 32)
					* Math.sin(2 * mu)
			...
			R1 = a * (1 - esq)
					/ Math.pow(1 - esq * Math.sin(phi1) * Math.sin(phi1), 1.5);
			D = x / (N1 * scale);
			phi = (phi1 - (N1 * Math.tan(phi1) / R1)
					* (D * D / 2
							- (5 + 3 * T1 + 10 * C1 - 4 * C1 * C1 - 9 * epsq)
							* D * D * D * D / 24 + (61 + 90 * T1 + 298 * C1
							+ 45 * T1 * T1 - 252 * epsq - 3 * C1 * C1)
							* D * D * D * D * D * D / 720))
					* RAD2DEG;
			lambda = (lambdaO + ((D - (1 + 2 * T1 + C1) * D * D * D / 6 + (5
					- 2 * C1 + 28 * T1 - 3 * C1 * C1 + 8 * epsq + 24 * T1 * T1)
	...
	public Point2D.Double forward(Point2D.Double lonLat) {
		...
		M = a
				* ((1 - esq / 4 - 3 * esq * esq / 64 - 5 * esq * esq * esq
						...
						* Math.sin(2 * phi)
						+ (15 * esq * esq / 256 + 45 * esq * esq * esq / 1024)
						* Math.sin(4 * phi) - (35 * esq * esq * esq / 3072)
						* Math.sin(6 * phi));
		x = scale
				* N
		...
		y = scale
				* (M - MO + N
						...
						* (A * A / 2 + (5 - T + 9 * C + 4 * C * C) * A * A * A
								* A / 24 + (61 - 58 * T + T * T + 600 * C - 330 * epsq)
								* A * A * A * A * A * A / 720)) + fn;
	...
	public Point2D.Double inverse(Point2D.Double xy) {
		...
		M = MO + y / scale;
		mu = M
				/ (a * (1 - esq / 4 - 3 * esq * esq / 64 - 5 * esq * esq * esq
						/ 256));
		...
		D = x / (N1 * scale);
		phi = (phi1 - (N1 * Math.tan(phi1) / R1)
				* (D * D / 2 - (5 + 3 * T1 + 10 * C1 - 4 * C1 * C1 - 9 * epsq)
						* D * D * D * D / 24 + (61 + 90 * T1 + 298 * C1 + 45
						* T1 * T1 - 252 * epsq - 3 * C1 * C1)
						* D * D * D * D * D * D / 720))
				* RAD2DEG;
		lambda = (lambdaO + ((D - (1 + 2 * T1 + C1) * D * D * D / 6 + (5 - 2
				* C1 + 28 * T1 - 3 * C1 * C1 + 8 * epsq + 24 * T1 * T1)
				* D * D * D * D * D / 120) / Math.cos(phi1)))
				* RAD2DEG;
	...
	public String getInverseJavaScript() {
		...
		sb.append("phi=(phi1-(N1*Math.tan(phi1)/R1)*(D*D/2-(5+3*T1+10*C1-4*C1*C1-9*epsq)*D*D*D*D/24+(61+90*T1+298*C1+45*T1*T1-252*epsq-3*C1*C1)*D*D*D*D*D*D/720))*"
				+ RAD2DEG + ";\n");
		sb.append("lambda=(lambdaO+((D-(1+2*T1+C1)*D*D*D/6+(5-2*C1+28*T1-3*C1*C1+8*epsq+24*T1*T1)*D*D*D*D*D/120)/Math.cos(phi1)))*"
				+ RAD2DEG + ";\n");
	...
	public static void main(String[] args) throws Exception {
		...
		imgs[0] = merc.getProjectedImage(50, 1000, 1000, pix, 2700, 1350, gr,
