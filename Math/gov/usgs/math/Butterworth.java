...
public class Butterworth
{
		...
		switch(type)
		{
			...
			case BANDPASS:
				...
				for (i = 0; i < sPlane.numPoles; i++)
				{
					...
					Complex temp = (new Complex(1.0, 0).minus(new Complex(new Complex(w0, 0).divide(hba)).sqr())).sqrt();
	...
	private void expandPoly()
	{
		...
		dcGain = Complex.evaluate(topCoeffs, zPlane.numZeros, botCoeffs, zPlane.numPoles, new Complex(1.0, 0));
		double theta = 2 * Math.PI * 0.5 * (rawAlpha1 + rawAlpha2); 
		fcGain = Complex.evaluate(topCoeffs, zPlane.numZeros, botCoeffs, zPlane.numPoles, Complex.expj(theta));
		hfGain = Complex.evaluate(topCoeffs, zPlane.numZeros, botCoeffs, zPlane.numPoles, new Complex(-1.0, 0));
