...
public class ImportEW extends Thread {
	...
	/**
	 * Try to recover from an {@link OutOfMemoryError} by dropping all {@link TraceBuf}s from the processing queue.
	 * Ideally, {@link OutOfMemoryError}s should be prevented by aggressive tuning; this
	...
	protected void processWinstonConfig() {
		...
		winston = new WinstonDatabase(winstonDriver, winstonURL, winstonPrefix, winstonStatementCacheCap);
		if (!winston.checkDatabase())
			fatalError("Winston database does not exist.");
		fixerWinston = new WinstonDatabase(winstonDriver, winstonURL, winstonPrefix, winstonStatementCacheCap);
	...
	class TraceBufHandler implements MessageListener {
		public void messageReceived(Message msg) {
			...
			if (matchingFilter != null) {
				...
				// Don't consume cycles on String.format if the message is not going
				// to be logged. Yes, it does matter. Running String.format for every
