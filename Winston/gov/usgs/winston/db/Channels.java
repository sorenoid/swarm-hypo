...
public class Channels 
{
	...
	/**
	 * Get List of channels
	 * @param fullMetadata if true, included full metadata
	 * @return List of channels
	 */
	public List<Channel> getChannels(boolean fullMetadata)
	{
		...
		try
		{
			...
			if (fullMetadata)
			{
				PreparedStatement ps = winston.getPreparedStatement("SELECT * FROM channelmetadata WHERE sid=? ORDER BY name ASC");
	...
	public boolean channelExists(String code)
	{
		...
		{
		    winston.getLogger().log(Level.SEVERE, "Could not determine channel existence.", Util.getLineNumber(this, e));
		}
		return false;
	}
	...
	public boolean createChannel(String code)
	{
		...
		{
			winston.useRootDatabase();
		    winston.getStatement().execute("INSERT INTO channels (code, st, et) VALUES ('" + code + "', 1E300, -1E300)");
		...
		{
		    winston.getLogger().log(Level.SEVERE, "Could not create channel.  Are permissions set properly?", Util.getLineNumber(this, e));
		}
		return false;
	}
	...
	public void updateInstrument(Instrument inst) {
		...
		try
		{
			...
			if (instrumentExists) {
				ps = winston.getPreparedStatement("UPDATE instruments SET name=?, description=?, lon=?, lat=?, height=? where iid=?;");
				ps.setInt(6, iid);
			} else {
				ps = winston.getPreparedStatement("INSERT INTO instruments (name, description, lon, lat, height) VALUES (?,?,?,?,?);");
			}
	...
	public List<Instrument> getInstruments()
	{
		...
		try
		{
			...
			PreparedStatement ps = winston.getPreparedStatement("SELECT * FROM instrumentmetadata WHERE iid=? ORDER BY name ASC");
