...
public class TraceBuf extends Message {
	...
	public void processBytes(DataInputStream in, boolean isTraceBuf2) throws IOException {
		...
		if (!isTraceBuf2) {
			channel = Util.bytesToString(buf, 16, 9).trim();
			location = null;
		} else {
	...
	public String toString() {
		// return (isTraceBuf2 ? "TYPE_TRACEBUF2" : "TYPE_TRACEBUF") + ": " + station + " " + channel + " " + network
		// + ", " + firstSampleTime + "->" + lastSampleTime() + "," + dataType + " " + quality + " " + numSamples;
		return String.format("TRACEBUF%s: %s %s %s %s, %d, %s, %.4f -> %.4f", isTraceBuf2 ? "2" : "", station, channel,
				network, ((isTraceBuf2 && location != null) ? location : "--"), numSamples, dataType, firstSampleTime(),
				lastSampleTime());
	}
